...
// ✅ Pagination setup for deleted products
...
$stmt1 = $conn->prepare("SELECT COUNT(*) AS total_records FROM products WHERE is_deleted = 1");
$stmt1->execute();
...
$stmt2 = $conn->prepare("SELECT * FROM products WHERE is_deleted = 1 LIMIT $offset, $total_records_per_page");
$stmt2->execute();
                        ...
                        <tbody>
                            <?php if ($products->num_rows > 0) { 
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="recover.php?recover_id=<?php echo $row['product_id']; ?>" 
                                           onclick="return confirm('Are you sure you want to recover this product?')">
                                           <button class="btn btn-success btn-sm">Recover</button>
                                        </a>
                                    </td>
                                </tr>
                            <?php } } else { ?>
                                <tr><td colspan="7" class="text-center">No deleted products found</td></tr>
                            <?php } ?>
                        </tbody>
                    </table>

                    <!-- ✅ Pagination -->
                    <nav aria-label="Deleted Products Page Navigation">
                        <ul class="pagination">
                            <li class="page-item <?php if($page_no <= 1){ echo 'disabled'; } ?>">
                                <a class="page-link" href="<?php if($page_no > 1){ echo "?page_no=".($page_no-1);} else {echo "#";} ?>">« Prev</a>
                            </li>
